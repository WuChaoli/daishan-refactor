...
4. **可验证** - 每条规则应可检查、可测试
5. **平台一致性** - 规则必须与 Codex CLI 能力和约束一致

## Codex 兼容要求

### 绝对禁令

- **禁止** 引用 Claude 专属机制：如 `AskUserQuestion`、`CLAUDE.md`、`.claude/settings.local.json`。
- **禁止** 编造不可用工具：规则中不得出现当前环境不存在的命令/工具。

### 必须遵守

- **必须** 使用 Codex 术语：如 `update_plan`、`apply_patch`、`exec_command`、`request_user_input`（仅 Plan 模式）。
- **必须** 体现 Codex 工作方式：先说明下一步，再执行工具调用；修改代码优先使用 `apply_patch`。
- **必须** 可落地验证：每条规则都应能通过命令、检查项或脚本进行验证。
...
通过逐条提问收集场景（一次一个问题；若在 Plan 模式可用 `request_user_input`）：
```markdown
- 哪些行为必须禁止？
- 哪些行为必须执行？
- 何时触发/调用？
...
## 必须遵守
- **必须**在工具调用前发送简短 preamble
- **必须**在多步骤任务中维护 `update_plan` 状态
- **必须**通过 `apply_patch` 修改文件
```
