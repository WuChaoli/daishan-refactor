...
async def intent_handler_1(data: dict, user_id: str, http_request: Request):
    ...
    try:
        ...
        if not results:
            logger.warning(f"Type1 处理失败: user_id={user_id}, results 为空")
            return StreamingResponse(
        ...
        if not answer:
            answer = "无效指令#@#@#"
            logger.warning(f"Type1 提取 answer 失败，使用默认兜底: user_id={user_id}")
        ...
        if not kb_matches:
            logger.info(f"Type1 命中普通指令: user_id={user_id}, answer_len={len(answer)}")
        ...
        logger.info(f"Type1 命中知识库指令: user_id={user_id}, kb={first_kb_name}")
        if first_kb_name:
...
{enterprise_stats}"""
                        ...
                        # 空值检查
                        if client is None:
                            logger.error(f"Type1 客户端为空: user_id={user_id}, kb={first_kb_name}")
                        ...
                        logger.error(f"Type1 流式处理异常: user_id={user_id}, reason={str(e)}")
                        # 发送错误事件
        ...
        logger.error(f"Type1 处理失败: user_id={user_id}, reason={str(e)}")
        return StreamingResponse(
...
async def intent_handler_2(data: dict, user_id: str, http_request: Request):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Type2 SQL查询完成: user_id={user_id}")
            ...
            if dify_chat_client:
                try:
                    ...
                    async def event_generator():
                        ...
                        try:
                            ...
                            if dify_chat_client is None:
                                logger.error(f"Type2 Dify客户端为空: user_id={user_id}")
                            ...
                            logger.error(f"Type2 Dify流式异常: user_id={user_id}, reason={str(e)}")
                            # 发送错误事件
                    ...
                    logger.warning(f"Type2 Dify包装失败，降级原始SQL结果: user_id={user_id}, reason={str(e)}")
                    pass
            else:
                pass
            ...
            logger.info(f"Type2 返回SQL降级结果: user_id={user_id}, result_len={len(result_str)}")
            return StreamingResponse(
            ...
            logger.error(f"Type2 DaiShanSQL导入失败: user_id={user_id}, reason={str(ie)}")
            return StreamingResponse(
        ...
        logger.error(f"Type2 处理失败: user_id={user_id}, reason={str(e)}")
        return StreamingResponse(
            content=convert_json_to_stream({"error": f"处理失败: {str(e)}"}),
            media_type="text/event-stream",
            headers={
            ...
            try:
                async with http_client.stream(
                    ...
                    if response.status_code != 200:
                        ...
                        break
                    yield f"event: start\\ndata: {json.dumps({'message': '开始流式输出'}, ensure_ascii=False)}\\n\\n"
                    logger.debug(f"Dify流式连接建立: status_code={response.status_code}")
                ...
                if retry_count > max_retry:
                    ...
                    logger.error(f"Dify流式请求超时终止: user_id={user_id}, retry_count={retry_count}")
                    yield f"event: error\\ndata: {json.dumps({'error': error_msg}, ensure_ascii=False)}\\n\\n"
                    break
                logger.warning(
                ...
                logger.error(f"Dify流式请求失败: user_id={user_id}, reason={str(e)}")
                yield f"event: error\\ndata: {json.dumps({'error': error_msg}, ensure_ascii=False)}\\n\\n"
                break

    except Exception as e:
        error_msg = f"处理请求时发生异常: {str(e)[:150]}"
        logger.error(f"Dify流式处理异常: user_id={user_id}, reason={str(e)}")
        yield f"event: error\\ndata: {json.dumps({'error': error_msg}, ensure_ascii=False)}\\n\\n"
...
async def instruct_process(query:str, http_request: Request):
    ...
    logger.info(f"指令预处理开始: query_len={len(query)}")

    # 空值检查
    if ragflow_client is None or config_manager is None:
        logger.error("指令预处理失败: 服务未完全初始化")
        raise HTTPException(
    ...
    try:
        ...
        logger.info(f"指令预处理意图结果: intent_type={intent_type}, result_count={len(result.results)}")
        ...
        if intent_type == 1:
            ...
            if "\tAnswer: " in question:
                ...
                if len(parts) == 2:
                    answer = parts[1]
            if not answer:
                logger.warning("指令预处理未提取到answer，返回兜底结果")
                return {
                    "answer": "无效指令#@#@#",
            ...
            logger.info("指令预处理非Type1，返回默认结果")
            return {
                "answer": instruction,
        ...
        logger.warning("指令预处理触发HTTPException，返回兜底结果")
        return {
            "answer": "无效指令#@#@#",
            "chat_type": "normal_instruct"
        }
...
async def root(request: Request):
    ...
    logger.info(f"根路径访问: uptime_seconds={uptime_seconds:.2f}")
    return {
        "service": "岱山意图识别服务",
        "version": "1.0.0",
        "status": "running",
...
async def health_check(request: Request):
    ...
    if ragflow_client is None:
        logger.warning("健康检查失败: ragflow_client 未初始化")
        return JSONResponse(
            status_code=503,
            content={
                "status": "unhealthy",
    ...
    try:
        ...
        if not ragflow_ok:
            ...
            logger.warning("健康检查降级: ragflow 连接失败")
    except Exception as e:
        health_status["checks"]["ragflow"] = {
            "status": "fail",
...
async def intent_recognition(request: IntentRequest, http_request: Request):
    ...
    try:
        ...
        # 根据路由到对应的处理器
        intent_type = result.type
        logger.info(f"意图识别完成: intent_type={intent_type}, result_count={len(result.results)}")
        ...
        if intent_type == 0:
            ...
            # 未知的 type,直接返回原始数据
            logger.warning(f"意图识别命中未知类型: intent_type={intent_type}")
            response = JSONResponse(content=response_data)

        return response

    except HTTPException:
        logger.warning("意图识别返回HTTPException")
        raise

    except Exception as e:
        logger.error(f"意图识别异常: reason={str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={
                "error": "internal_error",
...
async def instrcution_command(input_data: IntentRequest, request: Request):
    ...
    if not input_data.text_input.strip():
        logger.warning("流式聊天请求拒绝: text_input 为空")
    ...
    if len(result.results) > 0:
        ...
        logger.debug(f"流式聊天意图匹配: similarity={result.similarity}, question={best_result.question}")
