...
def get_enterprise_statistics(logger) -> str:
    """
    ...
    try:
        ...
        logger.info("开始调用 sql_introduction 获取企业统计数据")

        # 获取 DaiShanSQL 目录路径
        db_search_dir = Path(__file__).parent.parent / "DaiShanSQL"
        ...
        logger.info(f"sql_introduction 返回结果: {result}")
        ...
        if result.get("数据库查询状态") != "success":
            logger.error("sql_introduction 查询状态异常，数据库返回非 success 状态")
            return ""
        ...
        logger.info(f"企业统计数据获取成功，共 {len(stats_parts)} 条统计项")
        return "\n".join(stats_parts)

    except Exception as e:
        logger.error(f"获取企业统计数据失败: {e}", exc_info=True)
        return ""
...
async def intent_handler_0(data: dict, user_id: str, http_request: Request):
    ...
    """
    # 从 app.state 获取资源
    log_manager = getattr(http_request.app.state, 'log_manager', None)

    # 空值检查
    if log_manager is None:
        return StreamingResponse(
            content=convert_json_to_stream({"error": "LogManager not initialized"}),
    ...
    logger = log_manager.get_function_logger("intent_handler_0")
    query = data.get("query", "")
    logger.info(f"处理 Type 0 请求: {query[:50]}, user_id={user_id}")

    # 返回固定错误消息
    error_message = "无效指令#@#@#"
    logger.info(f"返回固定错误消息: {error_message}")
...
async def intent_handler_1(data: dict, user_id: str, http_request: Request):
    ...
    logger = log_manager.get_function_logger("intent_handler_1")
    logger.info(f"处理 Type 1 请求: {data.get('query', '')[:50]}, user_id={user_id}")

    try:
        # 检查 results 数组
        results = data.get("results", [])
        if not results:
            logger.warning("results 数组为空")
            return StreamingResponse(
        ...
        logger.info(f"第一个结果的 question: {question[:100]}")
        ...
        logger.info(f"提取的 Answer: {answer[:100]}")
        ...
        if not kb_matches:
            ...
            logger.info("未检测到知识库标记,直接返回 answer")
            return StreamingResponse(
        ...
        logger.info(f"检测到 {len(kb_matches)} 个知识库标记: {kb_matches}")
        ...
        logger.info(
            f"提取的 instruction: {instruction[:100] if instruction else '(空)'}..."
        )
                ...
                async def event_generator():
                    ...
                    try:
                        ...
                        if first_kb_name == "企业知识库":
                            logger.info("企业知识库请求，尝试获取企业统计数据")
                            enterprise_stats = get_enterprise_statistics(logger)
                            if enterprise_stats:
...
{enterprise_stats}"""
                                logger.info(
                                    f"已拼接企业统计数据，final_query 长度: {len(final_query)}"
                                )
                            else:
                                logger.info("未获取到统计数据，使用原始 query 继续")

                        # 调用 Dify Chat 流式接口
                        logger.info(
                            f"调用 {client_name} Dify Chat 流式接口, instruction 长度: {len(instruction) if instruction else 0}"
                        )
                ...
                # 客户端未初始化,使用降级方案
                logger.warning(f"知识库客户端未初始化,使用降级方案")
                return StreamingResponse(
            ...
            logger.warning("未检测到知识库标记,使用降级方案")
            return StreamingResponse(
                content=convert_json_to_stream({"result": answer}),
                media_type="text/event-stream",
                headers={
        ...
        logger.error(f"Type 1 处理失败: {str(e)}", exc_info=True)
        return StreamingResponse(
...
async def intent_handler_2(data: dict, user_id: str, http_request: Request):
    ...
    logger = log_manager.get_function_logger("intent_handler_2")
    logger.info(f"处理 Type 2 请求: {data.get('query', '')[:50]}, user_id={user_id}")
    ...
    try:
        ...
        try:
            ...
            if db_search_dir_str not in sys.path:
                sys.path.insert(0, db_search_dir_str)
                path_added = True
                logger.info(f"临时添加路径到 sys.path: {db_search_dir_str}")
            ...
            logger.info(
                f"调用 DaiShanSQL, questions: {questions}, conversation 长度: {len(history_list)}"
            )
            ...
            logger.info(f"DaiShanSQL 查询完成")

            # ======== DEBUG: 打印完整的 SQL 结果 ========
            logger.info(f"========== SQL Result Debug ==========")
            logger.info(f"SQL Result 类型: {type(sql_result)}")
            logger.info(f"SQL Result 内容: {sql_result}")
            if isinstance(sql_result, dict):
                logger.info(f"SQL Result 键: {list(sql_result.keys())}")
            ...
            if path_added and db_search_dir_str in sys.path:
                sys.path.remove(db_search_dir_str)
                logger.info(f"从 sys.path 移除路径: {db_search_dir_str}")

            # ======== Step 2: 尝试使用 Dify Chat 包装结果 ========
            if dify_chat_client:
                try:
                    logger.info("使用 Dify Chat 流式包装查询结果")
                    ...
                    logger.info(f"Query 长度: {len(formatted_query)} 字符")
                    ...
                    logger.warning(f"Dify Chat 调用失败,降级返回原始结果: {e}")
            else:
                logger.info("Dify Chat 客户端未初始化,使用降级模式")

            # ======== Step 3: 降级 - 返回原始 SQL 结果 ========
            logger.info("返回原始 SQL 查询结果")
            ...
            logger.error(f"导入 DaiShanSQL 模块失败: {str(ie)}", exc_info=True)
            return StreamingResponse(
        ...
        logger.error(f"Type 2 处理失败: {str(e)}", exc_info=True)
        return StreamingResponse(
            content=convert_json_to_stream({"error": f"处理失败: {str(e)}"}),
            media_type="text/event-stream",
            headers={
...
async def instruct_process(query:str, http_request: Request):
    ...
    # 空值检查
    if log_manager is None or ragflow_client is None or config_manager is None:
        raise HTTPException(
    ...
    start_time = time.time()
    logger = log_manager.get_function_logger("instruct_process")

    logger.info(
        f"收到请求，text_input='{query[:50]}...'"
    )
    try:
        # 执行意图判断
        ...
        # 检查日志容量并清理
        config = config_manager.get_config()
        if config and log_manager.check_total_size() > config.log_total_size_limit:
...
async def health_check(request: Request):
    ...
    # 空值检查
    if log_manager is None or ragflow_client is None:
        return JSONResponse(
            status_code=503,
            content={
                "status": "unhealthy",
                "checks": {
                    "log": {
                        "status": "pass" if log_manager is not None else "fail",
    ...
    start_time = time.time()
    logger = log_manager.get_function_logger("api_interface")
    logger.info("健康检查请求")
    ...
    # 检查日志容量
    log_size = log_manager.check_total_size()
    log_size_mb = log_size / (1024 * 1024)
    health_status["checks"]["logs"] = {
        "status": "pass" if log_size_mb < 400 else "warn",
    ...
    log_manager.log_timing("/health", "GET", elapsed_ms, status_code)
    logger.info(
        f"健康检查完成, status={health_status['status']}, 耗时={elapsed_ms:.2f}ms"
    )
...
async def intent_recognition(request: IntentRequest, http_request: Request):
    ...
    logger.info(
        f"收到请求，text_input='{request.text_input[:50]}...', user_id='{request.user_id}'"
    )
    ...
    try:
        ...
        log_manager.log_timing(
            "/intent",
            "POST",
            total_elapsed,
            response.status_code,
            extra=f"type={intent_type}, query={request.text_input[:30]}...",
        )
        ...
        # 记录错误请求的执行时间
        log_manager.log_timing(
            "/intent", "POST", elapsed, 500, extra=f"error={str(e)[:50]}"
        )
...
async def instrcution_command(input_data: IntentRequest, request: Request):
    ...
    logger.info(
        f"收到请求，text_input='{input_data.text_input[:50]}...', user_id='{input_data.user_id}'"
    )
