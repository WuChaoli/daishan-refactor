...
async def handle_personnel_dispatch(
    ...
    """
    logger = log_manager.get_function_logger("personnel_dispatch")

    try:
        logger.info(f"处理人员调度请求，语音文本: {voice_text}")

        # 获取 Dify clients
        general_client, personnel_client = _get_dify_clients(logger)

        # 步骤1: 实体提取
        entities = await _extract_entities(voice_text, general_client, logger)
        ...
        result_ids = await _call_personnel_dispatch_for_entities(
            entities, personnel_client, user_id, logger
        )
