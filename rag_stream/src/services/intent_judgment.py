...
async def intent_judgment(
    ...
    try:
        ...
        # 确定意图类型
        intent_type = settings.intent.default_type
        target_database = ""

        # 判断相似度是否达到阈值
        if best_result.total_similarity >= settings.intent.similarity_threshold:
            try:
                intent_type = settings.ragflow.get_intent_type(best_result.database)
            ...
            logger.warning(
                f"相似度 {best_result.total_similarity:.4f} < 阈值 {settings.intent.similarity_threshold},"
                f"降级到 type=0"
            )
            log_manager.log_function(
                "intent_judgment",
                ...
                threshold=settings.intent.similarity_threshold,
            )
